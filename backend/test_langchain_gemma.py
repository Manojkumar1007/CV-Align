#!/usr/bin/env python3
"""
Test script for LangChain + embeddingGemma chunking and embedding functionality in CV-Align RAG system.
This test requires Ollama to be running with the 'embeddinggemma:300m' model pulled.
"""
import asyncio
import os
import sys
...
def test_langchain_gemma_implementation():
    """Test the new LangChain + Gemma implementation."""
    print("Testing LangChain + Gemma implementation...")
    ...
    # Test basic text chunking with LangChain
    print("\n1. Testing LangChain text chunking...")
    try:
    ...
    # Test CV section chunking with LangChain
    print("\n2. Testing LangChain CV section chunking...")
    try:
        ...
        for i, chunk in enumerate(cv_chunks[:3]):  # Show first 3 chunks
        ...
        print(f"✗ Error in CV section chunking: {e}")
        return False
    ...
    try:
        ...
        print(f"✗ Error in RAG engine initialization: {e}")
        return False
    ...
    print("\n5. Testing RAG-enhanced evaluation with Gemma...")
    try:
...
if __name__ == "__main__":
    success = test_langchain_gemma_implementation()
    if not success:
        print("\n⚠ Tests failed. Check the above messages for details.")
        sys.exit(1)